...
/*下面兩段开始添加删除行**/
$(document).ready(function() {
    $("#todayTime").val(new Date().format("yyyy-MM-dd"));
    $.ajax({
        url:"../customer/all",
        cache:true,
        async:false,
    ...
    setTable($("#custName").val(),$("#custName :selected").text());
    trEdit();
...
function setTable(customerId,customerName){
    var htmlStr = "";
    $.ajax({
            ...
            if (data.length > 0 ) {
                data.forEach(function (item) {
                    htmlStr=htmlStr + "<tr >" +
                        ...
                        "<td  class='check-B'><input type='checkbox'></td>" +
